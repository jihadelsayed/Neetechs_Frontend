...
// }
public get products(): Observable<IProduct[]> {
  return of(this.currencyService.getCurrentCurrency()).pipe(
    map((currency:any) => {
      return product_data.map((product) => {
        return {
          ...product,
          price: this.convertCurrency(product.price, currency.code),
          // You may need to adjust other price-related properties here
        };
      });
    })
  );
}
private convertCurrency(price: number, targetCurrency: string): number {
  // Define conversion factors based on the target currency
  const conversionFactors:any = {
    'SEK': 10,     // Adjust these conversion factors based on your needs
    'USD': 1.2,
    'EUR': 1.0,
  };

  // Use the conversion factor for the target currency
  const conversionFactor = conversionFactors[targetCurrency] || 1.0;
